...
router.get('/', async (request, response) => {
    ...
    await axios
        .get(`https://dapi.kakao.com/v2/search/web?sort=${sort}&page=1&size=50&query=` + encodeURI(serch), {
            headers: {
        ...
        .then(async (res) => {
            if (res.data) {
                ...
                for (let i = 0; i < item.length; i++) {
                    ...
                    let date = Date(item[i].datetime)
                    let imgurl = htmlget(url)
                    let title = item[i].title.replaceAll('<b>', '').replaceAll('</b>', '').replaceAll('&#39;', '`').replaceAll('&#34;', '"')
                    let info = item[i].contents.replaceAll('<b>', '').replaceAll('</b>', '').replaceAll('&#39;', '`').replaceAll('&#34;', '"')
                    if (newkeyword > 0) {
                        result.push({
                            title: await title,
                            info: await info,
