...
const Date = async (date) => {
    const mmArry = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec']
    let arry = date.split(' ')
    let yy = arry[3]
    let mm = arry[2]
    for (let i = 0; i <= mmArry.length; i++) {
        if (mm == mmArry[i]) mm = i + 1
    }
    let dd = arry[1]
    let tt = arry[4].substr(0, 5)
    let Date = `${yy}.${mm}.${dd} ${tt}`
    return Date
}
...
router.get('/', async (request, response) => {
    ...
    await axios
        .get(`https://openapi.naver.com/v1/search/news?sort=${sort}&query=` + encodeURI(serch), {
            headers: {
        ...
        .then(async (res) => {
            if (res.data) {
                ...
                for (let i = 0; i < item.length; i++) {
                    ...
                    const date = Date(item[i].pubDate)
